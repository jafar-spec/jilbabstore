...
export const RESTOCK_COLLECTION = 'restock_requests';
export const RETURNS_COLLECTION = 'return_requests';
export const CARTS_COLLECTION = 'abandoned_carts';
...
// Returns a customer's orders matched BOTH by their account uid AND by their
// (verified) email — so guest/pre-signup orders placed with the same email also
// show up. Results are merged + de-duplicated. Each sub-query is independent
// (allSettled) so one being denied by rules doesn't drop the others.
export const getCustomerOrders = async (uid, email) => {
  const queries = [];
  if (uid) queries.push(getDocs(query(collection(db, ORDERS_COLLECTION), where('uid', '==', uid))));
  if (email) queries.push(getDocs(query(collection(db, ORDERS_COLLECTION), where('customerInfo.email', '==', email))));
  if (queries.length === 0) return [];
  try {
    const settled = await Promise.allSettled(queries);
    const map = new Map();
    settled.forEach(r => {
      if (r.status === 'fulfilled') r.value.docs.forEach(d => map.set(d.id, { ...d.data(), id: d.id }));
    });
    return [...map.values()].sort((a, b) => new Date(b.date || b.createdAt || 0) - new Date(a.date || a.createdAt || 0));
  } catch (error) {
...
export const subscribeToNewsletter = async (email, phone = '') => {
...
// A shopper asks to be told when a sold-out variant returns. Public create
// (like the newsletter); a server cron reads pending requests and notifies.
export const requestBackInStock = async ({ productId, productTitle, sku, size, color, email, phone }) => {
  if (!productId || (!email && !phone)) throw new Error('missing contact');
  const docRef = doc(collection(db, RESTOCK_COLLECTION));
  await setDoc(docRef, {
    productId, productTitle: productTitle || '', sku: sku || null,
...
// A customer requests to return a delivered order. Staff approve it in admin.
export const requestReturn = async ({ orderId, uid, reason, items }) => {
  if (!orderId) throw new Error('missing orderId');
  const docRef = doc(collection(db, RETURNS_COLLECTION));
  await setDoc(docRef, {
    orderId, uid: uid || null, reason: (reason || '').slice(0, 1000),
    items: items || [], status: 'pending', createdAt: new Date().toISOString(),
  });
  return docRef.id;
};

// Customer reads their own return requests (rules gate by uid).
export const getMyReturnRequests = async (uid) => {
  if (!uid) return [];
  const q = query(collection(db, RETURNS_COLLECTION), where('uid', '==', uid));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
};

// Admin: list all return requests (staff session).
export const getReturnRequests = async () => {
  const snap = await getDocs(collection(staffDb, RETURNS_COLLECTION));
  return snap.docs.map(d => ({ id: d.id, ...d.data() }))
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
};

// Admin: update a return request's status (pending → approved/rejected/done).
export const updateReturnRequest = async (id, data) => {
  await updateDoc(doc(staffDb, RETURNS_COLLECTION, id), data);
};

// --- Abandoned cart (logged-in shoppers only) ---
// Mirrors the cart to Firestore keyed by uid; a cron nudges stale carts. The
// doc is deleted on successful checkout (see clearCart wiring).
export const saveCartSnapshot = async (uid, { items, total, email, name, phone }) => {
  if (!uid) return;
  const docRef = doc(db, CARTS_COLLECTION, uid);
  if (!items || items.length === 0) { await deleteDoc(docRef).catch(() => {}); return; }
  await setDoc(docRef, {
    uid, email: email || '', name: name || '', phone: phone || '',
    items: items.map(i => ({ id: i.id, title: i.title, price: Number(i.price) || 0, quantity: i.quantity, image: i.image || null })),
    total: Number(total) || 0, updatedAt: new Date().toISOString(), reminded: false,
  }, { merge: true });
};

export const clearCartSnapshot = async (uid) => {
  if (!uid) return;
  await deleteDoc(doc(db, CARTS_COLLECTION, uid)).catch(() => {});
};

// --- Privacy: data export + deletion (customer self-service) ---
export const DELETION_REQUESTS_COLLECTION = 'deletion_requests';
...
// --- Address book (customer's own saved delivery addresses) ---
// Stored on customers/{uid}.addresses (rules already gate by owner). Each entry:
// { label, fullName, phone1, phone2, city, neighborhood, street, notes }.
// The default address is referenced by index so checkout can prefill instantly.
export const saveCustomerProfile = async (uid, data) => {
  if (!uid) return;
  await setDoc(doc(db, 'customers', uid), { ...data, updatedAt: new Date().toISOString() }, { merge: true });
};

export const saveCustomerAddresses = async (uid, addresses, defaultIndex = 0) => {
  if (!uid) return;
  const list = Array.isArray(addresses) ? addresses.slice(0, 10) : [];
  const idx = Math.min(Math.max(0, Number(defaultIndex) || 0), Math.max(0, list.length - 1));
  await setDoc(doc(db, 'customers', uid), {
    addresses: list, defaultAddressIndex: idx, updatedAt: new Date().toISOString(),
  }, { merge: true });
};

// Files a deletion request (admin completes the legal purge) and immediately
// clears what the customer owns directly (wishlist + cart snapshot). Orders are
// retained as required by tax/bookkeeping law and purged by staff if applicable.
export const requestAccountDeletion = async (uid, email) => {
  if (!uid) return;
  await setDoc(doc(db, DELETION_REQUESTS_COLLECTION, uid), {
    uid, email: email || '', status: 'pending', createdAt: new Date().toISOString(),
  }, { merge: true });
...
// Public: aggregate review ratings per product → { [productId]: { avg, count } }.
// Hidden reviews are excluded. Reads via the default (customer) app.
export const getReviewStats = async () => {
  try {
    ...
    snapshot.docs.forEach(d => {
      const r = d.data();
      if (r.isHidden || typeof r.rating !== 'number' || !r.productId) return;
      const cur = acc[r.productId] || { sum: 0, count: 0 };
      cur.sum += r.rating; cur.count += 1;
      acc[r.productId] = cur;
    });
    const stats = {};
    for (const [pid, { sum, count }] of Object.entries(acc)) {
      stats[pid] = { avg: sum / count, count };
    }
    return stats;
  } catch (e) {
    console.error('getReviewStats failed', e);
    return {};
  }
};
...
export const getPage = async (slug) => {
  try {
    const snap = await getDoc(doc(db, PAGES_COLLECTION, slug));
    return snap.exists() ? snap.data() : null;
  } catch (e) {
...
export const updatePage = async (slug, data) => {
  await setDoc(doc(staffDb, PAGES_COLLECTION, slug), { ...data, updatedAt: new Date().toISOString() }, { merge: true });
};

// Upsert the newsletter welcome coupon (first-order %) at a stable id so it
// stays in sync with the popup config and isn't duplicated.
export const upsertWelcomePromo = async (code, percent) => {
  const c = String(code).toUpperCase().trim();
  await setDoc(doc(staffDb, PROMOCODES_COLLECTION, c), {
    code: c, type: 'percent', discountValue: Number(percent) || 0,
    firstOrderOnly: true, active: true, source: 'newsletter-welcome'
  }, { merge: true });
};
