...
if (typeof window !== 'undefined' && process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY) {
  ...
  // App Check debug token — bypasses reCAPTCHA for local/testing sessions.
  // Enabled ONLY outside production (or when NEXT_PUBLIC_APPCHECK_DEBUG=true),
  // so production is never bypassable. Must be set BEFORE initializeAppCheck.
  // `true` → SDK prints a debug token in the console to register in
  // Firebase → App Check → Manage debug tokens. A fixed value can be pinned via
  // NEXT_PUBLIC_APPCHECK_DEBUG_TOKEN.
  const debugEnabled =
    process.env.NODE_ENV !== 'production' || process.env.NEXT_PUBLIC_APPCHECK_DEBUG === 'true';
  if (debugEnabled) {
